...
#include "client/world/mesher/ChunkMesher.h"
...
namespace Mineclone {
    ...
    void ChunkRenderSystem::update(Registry& registry, float dt) {
        ...
        for (auto it = m_chunkMeshData.begin(); it != m_chunkMeshData.end();) {
            if (!loadedChunks.contains(it->first)) {
        ...
        for (auto& [pos, chunk] : loadedChunks) {
            ...
            const auto& constWorld = static_cast<const World&>(m_world);
            std::array<const Chunk*, 4> neighbors = {
                constWorld.getChunk({pos.x, pos.z + 1}),
                constWorld.getChunk({pos.x, pos.z - 1}),
                constWorld.getChunk({pos.x + 1, pos.z}),
                constWorld.getChunk({pos.x - 1, pos.z})
            };
            ...
            auto [it, inserted] = m_chunkMeshes.try_emplace(pos, std::move(rawData), BlockVertex::layout());
            if (!inserted) {
