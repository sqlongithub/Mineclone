...
namespace Mineclone {
    ...
    public:
        ...
        const std::vector<VertexT>& getVertices(Layer layer) const {
            return m_layers[layerToIndex(layer)].vertices;
        }

        std::vector<VertexT>& getVertices(Layer layer) {
            return m_layers[layerToIndex(layer)].vertices;
        }

        const std::vector<uint32_t>& getIndices(Layer layer) const {
            return m_layers[layerToIndex(layer)].indices;
        }

        std::vector<uint32_t>& getIndices(Layer layer) {
            return m_layers[layerToIndex(layer)].indices;
        }
